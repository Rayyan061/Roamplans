param(
  [string]$TargetFolder,
  [string]$Category = 'Travel Technology'
)
...
foreach ($file in Get-ChildItem $folder -Filter '*.docx' | Sort-Object Name) {
  ...
  $inList = $false; $inFaq = $false; $faqOpen = $false; $inFinalThoughts = $false; $seenTitle = $false; $headingIndex = 0; $tableImageAdded = $false; $sectionImageAdded = $false
  ...
  foreach ($item in $items) {
    ...
    if ($item.Text -eq 'Final Thoughts') {
      ...
      [void]$body.AppendLine('<div class="final-thoughts-box">')
      $inFinalThoughts=$true
  ...
  if ($inFinalThoughts) { [void]$body.AppendLine('</div>') }
...
<main class="rp-final-main"><article class="rp-post"><header class="rp-hero"><div class="hero-inner"><div class="eyebrow">RoamPlans &middot; $(Encode $Category)</div><h1>$(Encode $title)</h1><div class="hero-meta"><span>&#10003; Practical guidance</span><span>&#8982; International travel</span><span>&#10022; Updated August 2026</span></div></div></header>
